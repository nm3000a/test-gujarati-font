...
main(void)
{
    ...
    /* GUI */
    xw.font = nk_xfont_create(xw.dpy, "Noto Serif Gujarati");
    ...
    while (running)
    {
        ...
        /* GUI */
        /* if (nk_begin(ctx, " ડેમો", nk_rect(50, 50, 200, 200), */
            ...
            NK_WINDOW_CLOSABLE|NK_WINDOW_MINIMIZABLE|NK_WINDOW_TITLE))
        {
            ...
            if (nk_button_label(ctx, "બટન "))
                fprintf(stdout, "બટન  દબાવ્યુ\n");
            nk_layout_row_dynamic(ctx, 30, 2);
            if (nk_option_label(ctx, "સહેલું", op == EASY)) op = EASY;
            if (nk_option_label(ctx, "અઘરું ", op == HARD)) op = HARD;
            nk_layout_row_dynamic(ctx, 25, 1);
            nk_property_int(ctx, "કોમ્પ્રેશન :", 0, &property, 100, 10, 1);
        }
        nk_end(ctx);
